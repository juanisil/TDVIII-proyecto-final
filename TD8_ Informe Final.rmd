...
A partir de la pregunta de la investigación, se plantea el problema de encontrar el jugador ideal para un equipo de fútbol. En un comienzo nos encontramos planteando cómo definir la _performance_ de un jugador y cómo compararla con otros jugadores. Surgió la necesidad de encontrar una métrica para evaluar el impacto de un jugador en el rendimiento de un equipo y cómo definir estos agentes. Además es necesario poder representar concretamente a un Jugador $J$.
...
En un comienzo se planteó desarrollar un modelo predictivo para estimar los ratios de transición entre los estados. Optamos por buscar predecir los ratios $r$ y no las probabilidades de transición $q$ ya que la normalización no es igual en cada instancia de $R$. Más concretamente buscamos estimar la función $f$ que mapea los estados $U$ y $V$ al ratio de transición $r(U, V)$.
...
Para el modelo predictivo comenzamos utilizando un modelo de XGBoost para la regresión pero rápidamente observamos que por la naturaleza de árbol al predecir con la media de las observaciones por hoja las predicciones resultaban casi discretas, por lo que viramos a explorar un modelo de regresión lineal para predecir los ratios de pases entre jugadores.
...
En la figura \ref{fig:regresion_lineal_r_p} podemos observar como a pesar de predecir muy pobre los ratios de transición al resultar en un coeficiente de correlación de Pearson entre los $r(J_i, J_j)$ y los $\hat{r}(J_i, J_j)$ de $0.12$, sin embargo al comparar el PSL real del PSL calculado a partir de $\hat{R}$ se obtiene un coeficiente de correlación de Pearson de $0.85$.
...
\begin{figure}
  ...
  \caption{Resultados Modelo de Regresión Lineal}\label{fig:regresion_lineal_r_p}
\end{figure}
...
Pudimos observar que las transiciones de Jugador a Shot son las que más inciden sobre el PSL, seguido por las transiciones entre jugadores, tal como se observa en la figura \ref{fig:psl_gradient}.

<!-- ![Gradiente del PSL](recursos_pdf/graficos/psl_gradient.jpeg) -->

\begin{figure}
  \includegraphics{recursos_pdf/graficos/psl_gradient.jpeg}
  \caption{Gradiente del PSL}\label{fig:psl_gradient}
\end{figure}
...
\begin{figure}
  ...
  \caption{Resultados Modelo de XGBoost}\label{fig:xgb_shots}
\end{figure}
...
A partir de la distribución ajustada de un jugador, podemos hallar jugadores similares en base a la distribución de los $r(J, S)$ utilizando la divergencia de Kullback-Leibler (KL). 
...
Dado un equipo $A$ con una formación de 11 jugadores $L_{A}$, se busca estimar la distribución del PSL de ese equipo a partir de las distribuciones _a-priori_ de los $r(U, V)$ de cada jugador. Para ello, se propone un proceso de Monte Carlo para muestrear de las distribuciones _a-priori_ de los $r(U, V)$.
...
El grafo resultante de la composición de todos los grafos de lineup $G_{\text{Full}}$ se puede comprender mejor en la visualización presente en la figura \ref{fig:G_EPL_12_13}:
...
Donde al igual que en la figura anterior (\ref{fig:G_LTM}), los nodos de jugadores reales se encuentran representados por los nodos de los lineups en los que participaron.
...
Se entrenó el modelo y se obtuvieron los embeddings de los 521 jugadores de la temporada 2012/13 de la EPL. La siguiente visualización en la figura \ref{fig:players_3d} muestra los embeddings de los jugadores en un espacio de 3 dimensiones, el color corresponde al equipo en el que juega el jugador.

<!-- ![Embeddings de Jugadores en 3D](recursos_pdf/graficos/players_3d.png) -->
\begin{figure}
  \includegraphics{recursos_pdf/graficos/players_3d.png}
  \caption{Embeddings de Jugadores en 3D}\label{fig:players_3d}
\end{figure}

Para poder visualizar de forma más clara los embeddings de los jugadores, se realizó un PCA para reducir la dimensionalidad de los embeddings a 2 dimensiones. La visualización presente en la figura \ref{fig:players_3d_2d} muestra los embeddings de los jugadores en un espacio de 2 dimensiones, el color corresponde al equipo en el que juega el jugador.
...
\begin{figure}
  ...
  \caption{Embeddings de Jugadores en 3D - PCA a 2D}\label{fig:players_3d_2d}
\end{figure}


En la figura \ref{fig:players_3d_2d} se observa cómo los jugadores de un mismo equipo se encuentran cercanos en el espacio vectorial, lo que indica que los embeddings resultantes de este modelo capturan las relaciones entre los jugadores de un mismo equipo.
...
\begin{figure}
  ...
  \caption{Embeddings de Jugadores en 64D - PCA a 2D}\label{fig:players_64_2d}
\end{figure}
...
\begin{figure}
  ...
  \caption{Comparación de CDFs y PDFs de Welbeck y Walcott}\label{fig:Welbeck_Walcott}
\end{figure}
...
\begin{figure}
  ...
  \caption{Comparación de CDFs y PDFs de Welbeck y Giroud}\label{fig:Welbeck_Giroud}
\end{figure}
...
<!-- Appearances: Welbeck made 126 appearances for Arsenal across all competitions.
Goals: He scored a total of 32 goals, averaging approximately 0.25 goals per game.
Assists: He provided 13 assists, contributing to the team's overall attacking play.
Notable Matches: His debut came against Manchester City, where he scored his first goal in a 3–0 victory over Aston Villa shortly after joining. He also scored a hat-trick in a UEFA Champions League match against Galatasaray, showcasing his potential as a key player for the Gunners23.
Injury Challenges
Welbeck's time at Arsenal was significantly impacted by injuries.
He suffered a serious knee injury that sidelined him for extended periods, including missing the entire second half of the 2015 season and parts of subsequent seasons. These injuries limited his ability to maintain consistent form and playing time24.
Impact on Team Success
Despite his injury struggles, Welbeck played crucial roles in important matches:
He scored the winning goal against Manchester United in the FA Cup quarter-finals in 2015, helping Arsenal progress to the later stages of the tournament.
He was part of the squad that won the FA Cup in 2017, contributing to Arsenal's success during his tenure23.

Performance at Arsenal
Initial Impact: Welbeck started strong, scoring his first goal shortly after joining and netting a hat-trick against Galatasaray in the Champions League, which showcased his potential as a key player for the team13. His ability to combine effectively with teammates, particularly Mesut Özil, highlighted his strengths in creating scoring opportunities1.
Injury Issues: However, his time at Arsenal was marred by significant injury problems. He missed substantial portions of multiple seasons due to knee injuries, which limited his contributions and consistency on the pitch3. This recurring issue prevented him from establishing himself as a regular starter and hindered his overall impact on the team's success.
Comparison with Manchester United
At Manchester United, Welbeck had a promising start, contributing to the team's successes, including winning the League Cup and FIFA Club World Cup. He was known for his versatility and work rate but often played second fiddle to more established strikers like Wayne Rooney and Robin van Persie. His time at United was characterized by:
Development: He developed as a player under Sir Alex Ferguson, gaining valuable experience through loan spells and eventually becoming a part of the first team.
Limited Goalscoring: While he showed flashes of brilliance, his goalscoring record was inconsistent, leading to mixed opinions about his finishing ability.
-->
...
Se puede considerar también su valor de mercado en el momento de la transferencia, que fue de 20 millones de libras esterlinas, como un indicador de su potencial y calidad como jugador. De la misma forma, podemos observar en la figura \ref{fig:Welbeck_Market_Value} que durante su tiempo en el Arsenal, su valor de mercado fue disminuyendo luego de un año transcurrido en el club, lo que podría ser un indicador de su rendimiento decreciente, posiblemente debido a las lesiones [@danny_transfermarkt]. Su valor de mercado al momento de su transferencia de salida del Arsenal fue de 9 millones de libras esterlinas.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{recursos_pdf/graficos/evolucion_valor_mercado_wellbeck.png}
  \caption{Valor de Mercado de Danny Welbeck en el Arsenal - Fuente: Transfermarkt}\label{fig:Welbeck_Market_Value}
\end{figure}
...
\begin{figure}
  ...
  \caption{Comparación de CDFs y PDFs de Milner y Gerrard}\label{fig:Milner_Gerrard}
\end{figure}
...
\begin{figure}
  ...
  \caption{Comparación de CDFs y PDFs de Milner y Downing}\label{fig:Milner_Downing}
\end{figure}
...
\begin{figure}
  ...
  \caption{Distribución de los $r(J, S)$ de los 10 jugadores con mayor cantidad de disparos}\label{fig:Top_10_by_counts_players_shots_prob_beta_binomial}
\end{figure}
...
\begin{figure}
  ...
  \caption{Distribución de los $r(J, S)$ de los 10 jugadores con mayor sesgo}\label{fig:Top_10_by_skew_players_shots_prob_beta_binomial}
\end{figure}
...
\begin{figure}
  ...
  \caption{Top 20 Delanteros con distribución Beta más sesgada a la derecha - EPL 12/13}\label{fig:Top_20_Forwards_by_skew_players_shots_prob_beta_binomial}
\end{figure}
...
\begin{figure}
  ...
  \caption{Distribución de los $r(J, S)$ de los 10 jugadores con mayor suma}\label{fig:Top_10_by_sum_players_shots_prob_beta_binomial}
\end{figure}
