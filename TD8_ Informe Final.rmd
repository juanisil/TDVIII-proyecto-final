...
Dado un equipo $A$ con una formación de 11 jugadores, se busca estimar la distribución del PSL de ese equipo a partir de las distribuciones a prior de los $r(U, V)$ de cada jugador. Para ello, se propone un proceso de Monte Carlo para muestrear de las distribuciones a priori de los $r(U, V)$.
...
\KwIn{Matriz de variables aleatorias $\mathbf{R}$}
\KwOut{Distribución del PSL del equipo $A$}
Inicializar $N$ simulaciones\;
...
\begin{algorithm}[H]
\caption{Comparación de Distribuciones de PSL}\label{alg:cap2}
\SetAlgoLined
\KwIn{Formación $L_A$ = $\{J_1, J_2, \dots, J_{11}\}$, Donde algún $J_i$ es el jugador a "original"}
\KwIn{Jugador $J'$ a comparar con $J_i$}
\KwIn{Formación $L'_A$ = $\{J_1, J_2, \dots, J_{11}}\}$
\KwOut{Distribuciones del PSL de $L_A$ con $J_i$ y de $L'_A$}
Inicializar $N$ simulaciones\;
}
Estimar la distribución del PSL del equipo $A$ al reemplazar a $J_1$ por $J_2$ a partir de las $N$ observaciones obtenidas de las simulaciones\;
\end{algorithm}
